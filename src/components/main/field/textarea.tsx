...
export default function Textareafield({ node }: { node: Node }) {
...
const onsubmit =()=>{
  if  (property?.properties.label !== label ||property?.properties.text !== text ||property?.properties.helpertext !== helpertext ||property?.properties.required !== required||property?.properties.size !== size) {
  ...
  <div className='w-full mt-11 flex flex-col gap-9'>
     ...
     <div className='text-slate-500 mt-2'>The place holder of this field </div>
      ...
      <div className='flex flex-col gap-2'>
         ...
         <Textarea className='w-full' value={helpertext} placeholder={helpertext||"Enter helpertext"} onChange={(e)=>sethelpertext(e.target.value)}  />
         <div className='text-slate-500 mt-2'>The helper text will display below main text field</div>

      </div>

      <div className='flex flex-col gap-2'>
      <Label htmlFor="title">{`No of rows:${size}`}</Label>
      <Slider
          defaultValue={[size]}
          max={10}
          step={1}
